...
structure Project {
    ...
    @documentation("Security group IDs associated with the project")
    @jsonName("security_groups")
    // AWS default limit: 5 security groups per network interface
    @length(max: 5)
    securityGroups: StringList

    @documentation("IAM policy ARNs associated with the project")
    @jsonName("policy_arns")
    // AWS default limit: 10 managed policies per IAM role
    @length(max: 10)
    policyArns: PolicyArnList

    @documentation("Project scripts configuration")
    scripts: ProjectScripts

    @documentation("Project tags")
    tags: TagList
}

@documentation("Budget information structure")
structure Budget {
    @documentation("Budget name")
    @jsonName("budget_name")
    name: String
}

@documentation("Project scripts configuration per OS")
structure ProjectScripts {
    @documentation("Linux script events")
    linux: ScriptEvents

    @documentation("Windows script events")
    windows: ScriptEvents
}

@documentation("Script event configuration")
structure ScriptEvents {
    @documentation("Scripts to run on VDI start")
    @jsonName("on_vdi_start")
    onVdiStart: ScriptList

    @documentation("Scripts to run on VDI configured")
    @jsonName("on_vdi_configured")
    onVdiConfigured: ScriptList

    @documentation("Whether to rerun scripts on reboot")
    @jsonName("rerun_on_reboot")
    rerunOnReboot: Boolean
}

@documentation("Script definition")
structure Script {
    @documentation("Location of the script. Supported formats: S3 URI (s3://bucket/script.sh), HTTPS URL (https://example.com/script.sh), or local file (file:///path/to/script.sh)")
    @jsonName("script_location")
    scriptLocation: String

    @documentation("Arguments to pass to the script")
...
@pattern("^arn:(aws|aws-cn|aws-us-gov):iam::[0-9]{12}:policy/.+$")
...
// AWS limit: 50 tags per EC2 resource
@length(max: 50)
...
structure VirtualDesktopSessionScreenshot {
    @documentation("IDEA session identifier")
    @jsonName("idea_session_id")
    @required
    @length(min: 1)
    ...
    @documentation("Image format. Supported formats: jpeg, png")
    format: String

    @documentation("Base64 encoded image data")
    ...
    @documentation("Failure reason if the screenshot could not be retrieved")
    @jsonName("failure_reason")
    failureReason: String
}

@documentation("List of virtual desktop session screenshots")
list VirtualDesktopSessionScreenshotList {
    member: VirtualDesktopSessionScreenshot
}
...
structure VirtualDesktopSessionConnection {
    ...
    @documentation("Connection endpoint URL")
    endpoint: String
