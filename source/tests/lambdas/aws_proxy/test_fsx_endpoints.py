...
from tests.lambdas.util import post_api_response, post_api_response_data
...
@pytest.fixture
def describe_storage_virtual_machines_headers() -> dict[str, str]:
    return {
...
@pytest.fixture
def describe_volumes_headers() -> dict[str, str]:
    return {
...
def test_describe_virtual_machines(
    url_path: str, describe_storage_virtual_machines_headers: Any, request_body: Any
) -> None:
    """
    Test the DescribeStorageVirtualMachines API call.
    """
    response_data = post_api_response_data(
        url_path, describe_storage_virtual_machines_headers, request_body
    )

    # Add assertions based on the expected response structure
    assert "StorageVirtualMachines" in response_data
    assert isinstance(response_data["StorageVirtualMachines"], list)


def test_describe_volumes(
    url_path: str, describe_volumes_headers: Any, request_body: Any
) -> None:
    """
    Test the DescribeVolumes API call.
    """
    response_data = post_api_response_data(
        url_path, describe_volumes_headers, request_body
    )

    # Add assertions based on the expected response structure
    assert "Volumes" in response_data
    assert isinstance(response_data["Volumes"], list)
