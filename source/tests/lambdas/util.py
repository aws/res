...
def put_api_response_data(url_path: str, headers: Any, request_body: Any) -> Any:
    """
    Makes a PUT API request with the provided headers and request body, and returns the JSON response data.
    """
    response = put_api_response(url_path, headers, request_body)
    response.raise_for_status()  # Raise an exception for non-2xx status codes
...
def put_api_response(url_path: str, headers: Any, request_body: Any) -> Any:
    """
    Makes a PUT API request with the provided headers and request body, and returns the response object.
    """
    base_url = os.environ.get("BASE_URL")
    if not base_url:
        raise ValueError("BASE_URL environment variable is not set")
    ...
    url = f"{base_url}{url_path}"
    response = requests.put(url, headers=headers, json=request_body, verify=False)
    return response
