...
def sfn_client(
    ...
    monkeypatch.setenv(handlers.EnvKeys.SFN_ARN, sfn_arn)
...
def expected_status(error: Optional[str]) -> str:
    if error is not None:
        return handlers.CustomResourceResponseStatus.FAILED
    return handlers.CustomResourceResponseStatus.SUCCESS
...
def test_handle_custom_resource_lifecycle_event(
    ...
    handlers.handle_custom_resource_lifecycle_event(event, None)
    ...
    assert_made_request_matches(
        ...
        expected_data=handlers.CustomResourceResponse(
...
def test_handle_custom_resource_delete_lifecycle_event(
    ...
    event = dict(
        ...
        StackId="test_logical_stack_id",
        RequestId="test_logical_request_id",
        RequestType=handlers.RequestType.DELETE,
        ResponseURL=callback_url,
    )
...
def wait_condition_response_event(
    ...
    event: Dict[str, Any] = dict(
        ResourceProperties={handlers.EnvKeys.CALLBACK_URL: callback_url},
    )

    if error is not None:
        event[handlers.EnvKeys.ERROR] = error
...
def wait_condition_expected_status(error: Optional[str]) -> str:
    if error is not None:
        return handlers.WaitConditionResponseStatus.FAILURE
    return handlers.WaitConditionResponseStatus.SUCCESS
...
def test_send_wait_condition_response(
    ...
    handlers.send_wait_condition_response(wait_condition_response_event, None)

    assert_made_request_matches(
        mock_urlopen=mock_urlopen,
        expected_url=callback_url,
        expected_data=handlers.WaitConditionResponse(
...
def test_send_wait_condition_delete_response(
    ...
    handlers.send_wait_condition_response(event, None)
