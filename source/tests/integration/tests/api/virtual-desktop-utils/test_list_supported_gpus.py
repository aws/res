...
"""
Integration tests for Virtual Desktop Supported GPU API.

This module contains comprehensive tests for the virtual desktop supported GPU REST API
...
class TestListSupportedGPUs:
    """Test suite for the list supported GPUs endpoint."""

    @pytest.mark.parametrize("admin_username", ["clusteradmin"])
    def test_list_supported_gpus_returns_valid_response(
        self,
        ...
        """
        Test successful retrieval of supported GPUs.
        """
        try:
            api_client = ApiClient(res_environment, admin)
            response = api_client.list_supported_gpus()
            ...
            # Verify each item in the listing is a valid VirtualDesktopGpu enum value
            valid_gpu_values = [gpu.value for gpu in VirtualDesktopGpu]
            for gpu_item in response.listing:
                assert (
                    gpu_item in valid_gpu_values
                ), f"GPU '{gpu_item}' is not a valid VirtualDesktopGpu enum value. Valid values: {valid_gpu_values}"

            logger.info(
                f"Successfully retrieved {len(response.listing)} supported GPUs: {response.listing}"
            )
    ...
    def test_list_supported_gpus_with_non_admin_user_returns_authorization_error(
        self,
        request: FixtureRequest,
        region: str,
        res_environment: ResEnvironment,
        ...
        """
        Test that non-admin users get proper authorization error for supported GPU endpoint.
        """
        try:
            api_client = ApiClient(res_environment, non_admin)
            api_client.list_supported_gpus()
            ...
            if "Unauthorized user" in response_content:
                logger.info(
                    "Non-admin user correctly received 'Unauthorized user' error for supported GPU endpoint"
                )
                return  # Test passes

            pytest.fail(f"Unexpected error for non-admin user: {str(e)}")

    def test_list_supported_gpus_with_nonexistent_user_returns_user_not_found_error(
        self, res_environment: ResEnvironment
    ) -> None:
        """
        Test that non-existent users get proper 'User not found' error for supported GPU endpoint.
        ...
        try:
            ...
            if "401" in str(e):
                ...
                if "User not found" in response_content:
                    logger.info(
                        "Non-existent user correctly received 'User not found' error for supported GPU endpoint"
                    )
                    return  # Test passes
    ...
    def test_list_supported_gpus_with_inactive_user_returns_inactive_user_error(
        self,
        request: FixtureRequest,
        region: str,
        res_environment: ResEnvironment,
        ...
        """
        Test that inactive users get proper 'Inactive user' error for supported GPU endpoint.
        ...
        try:
            ...
            if "401" in str(e):
                ...
                if "Inactive user" in response_content:
                    logger.info(
                        f"Inactive user correctly received 'Inactive user' error for supported GPU endpoint: {e.response.text}"
                    )
                    return  # Test passes

            pytest.fail(f"Unexpected error for inactive user: {str(e)}")

    def test_list_supported_gpus_without_auth_token_returns_no_authorization_token_provided_error(
        self,
        request: FixtureRequest,
        region: str,
        res_environment: ResEnvironment,
        ...
        """
        Test that requests without auth tokens get proper 'No authorization token provided' error for supported GPU endpoint.
        ...
        try:
            ...
            if "401" in str(e):
                ...
                if "No authorization token provided" in response_content:
                    logger.info(
                        f"Request without auth token correctly received 'No authorization token provided' error for supported GPU endpoint: {e.response.text}"
                    )
                    return  # Test passes
    ...
    @pytest.mark.parametrize("admin_username", ["clusteradmin"])
    def test_list_supported_gpus_in_prod_with_invalid_auth_token_returns_authorization_error(
        self,
        request: FixtureRequest,
        region: str,
        res_environment: ResEnvironment,
        ...
        """
        Test that requests with invalid auth tokens get proper 'Unable to retrieve username' error for supported GPU endpoint.
        ...
        try:
            ...
            if "401" in str(e):
                ...
                if "Unable to retrieve username" in response_content:
                    logger.info(
                        f"Request with invalid auth token correctly received 'Unable to retrieve username' error for supported GPU endpoint: {e.response.text}"
                    )
                    return  # Test passes
