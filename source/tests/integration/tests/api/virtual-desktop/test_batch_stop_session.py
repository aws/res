...
class TestBatchStopSession:

    def _build_request(self, session_records: List[Dict[str, Any]]) -> Any:
        """Build a BatchStopSessionRequestContent from session records."""
        session_list = [
        ...
        return BatchStopSessionRequestContent(sessions=session_list)
    ...
    @pytest.mark.parametrize(
        ...
        [
            [
                {
                    "session_id": "test-batch-stop-admin-own",
                    sessions.SESSION_DB_HASH_KEY: "clusteradmin",
                },
                {
                    "session_id": "test-batch-stop-admin-other",
    ...
    def test_batch_stop_session_with_admin_user(
        self,
        region: str,
        ...
        """Test that admin can stop both own and other user's sessions."""
        ...
        try:
            ...
            response = api_client.batch_stop_session(request_content)

            assert response is not None, "Response should not be None"
            assert (
                len(response.successful_list) == 2
            ), "Admin should successfully stop both sessions"
            successful_ids = {s.idea_session_id for s in response.successful_list}
            assert "test-batch-stop-admin-own" in successful_ids
            assert "test-batch-stop-admin-other" in successful_ids
            assert (
                len(response.unsuccessful_list) == 0
            ), "There should be no unsuccessful sessions"

            logger.info("Admin successfully stopped both own and other user's sessions")
        finally:
    ...
    @pytest.mark.parametrize(
        ...
        [
            [
                {
                    "session_id": "test-batch-stop-user1-123",
                    sessions.SESSION_DB_HASH_KEY: "user1",
                }
            ]
        ],
    ...
    def test_batch_stop_session_with_non_admin_user(
        self,
        ...
        try:
            ...
            assert (
                ...
                == "test-batch-stop-user1-123"
            )
            assert (
                len(response.unsuccessful_list) == 0
            ), "There should be no unsuccessful sessions"

            logger.info("Non-admin user successfully stopped their own session")
        finally:
            set_backend_lambda_dry_mode(region, environment_name, False)

    def test_batch_stop_session_with_nonexistent_user(
        self,
        request: FixtureRequest,
        region: str,
        res_environment: ResEnvironment,
        ...
        try:
            ...
            request_content = BatchStopSessionRequestContent(sessions=session_list)
            api_client.batch_stop_session(request_content)
    ...
    def test_batch_stop_session_with_inactive_user(
        self,
        region: str,
        res_environment: ResEnvironment,
    ...
    def test_batch_stop_session_without_auth_token(
        self,
        res_environment: ResEnvironment,
        region: str,
    ) -> None:
    ...
    def test_batch_stop_session_with_invalid_auth_token(
        self,
        region: str,
        res_environment: ResEnvironment,
        environment_name: str,
    ...
    @pytest.mark.parametrize(
        ...
        [
            [
                {
                    "session_id": "test-batch-stop-other-user-123",
    ...
    @pytest.mark.parametrize("non_admin_username", ["user1"])
    def test_batch_stop_session_non_admin_other_users_session(
        self,
        request: FixtureRequest,
        region: str,
        res_environment: ResEnvironment,
        ...
        """Test that non-admin user cannot stop another user's session."""
        api_client = ApiClient(res_environment, non_admin)
        request_content = self._build_request(session_record)
        response = api_client.batch_stop_session(request_content)
    ...
    @pytest.mark.parametrize("admin_username", ["clusteradmin"])
    def test_batch_stop_session_nonexistent_session(
        self,
        region: str,
        res_environment: ResEnvironment,
        admin_username: str,
        ...
        """Test batch stop with a session ID that does not exist in DDB."""
        api_client = ApiClient(res_environment, admin)
        session_list = [
            VirtualDesktopSession(
        ...
        assert (
            response.unsuccessful_list[0].error_code
            == BatchOperationErrorCode.NOTFOUNDEXCEPTION
        ), f"Expected NotFoundException, got: {response.unsuccessful_list[0].error_code}"
    ...
    @pytest.mark.parametrize("admin_username", ["clusteradmin"])
    def test_batch_stop_session_empty_sessions_list(
        self,
        region: str,
        res_environment: ResEnvironment,
        admin_username: str,
        ...
        try:
            api_client = ApiClient(res_environment, admin)
            request_content = BatchStopSessionRequestContent(sessions=[])
            api_client.batch_stop_session(request_content)
    ...
    @pytest.mark.parametrize("admin_username", ["clusteradmin"])
    def test_batch_stop_session_missing_sessions_field(
        self,
        region: str,
        res_environment: ResEnvironment,
        admin_username: str,
        ...
        try:
            api_client = ApiClient(res_environment, admin)
            request_content = BatchStopSessionRequestContent(sessions=None)
            api_client.batch_stop_session(request_content)
    ...
    @pytest.mark.parametrize("admin_username", ["clusteradmin"])
    def test_batch_stop_session_missing_idea_session_id(
        self,
        region: str,
        res_environment: ResEnvironment,
        admin_username: str,
