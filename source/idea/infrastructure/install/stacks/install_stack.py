...
from idea.infrastructure.install.constants import (
    ...
    SHARED_RES_LIBRARY_LAMBDA_LAYER_RUNTIME,
)
...
class InstallStack(Stack):
    def __init__(
        ...
        self.res_base_stack = ResBaseStack(self, self.parameters)
    ...
    def create_shared_res_library_lambda_layer(self) -> lambda_.LayerVersion:
        ...
        shared_library_layer = lambda_.LayerVersion(
            ...
            code=lambda_.Code.from_custom_command(
                ...
                command=["invoke clean.library build.library package.library"],
                command_options={"stdio": "inherit", "shell": True},
                bundling=aws_cdk.BundlingOptions(
                    image=SHARED_RES_LIBRARY_LAMBDA_LAYER_RUNTIME.bundling_image,
                    command=[
                        f"dir=/asset-output/python/lib/{SHARED_RES_LIBRARY_LAMBDA_LAYER_RUNTIME.name}/site-packages && mkdir -p $dir && pip3 install -r requirements.txt -t $dir && pip3 install *.tar.gz -t $dir",
                    ],
                    user="root",
                    entrypoint=["/bin/sh", "-c"],
                    output_type=aws_cdk.BundlingOutput.NOT_ARCHIVED,
                ),
            ),
            compatible_runtimes=[SHARED_RES_LIBRARY_LAMBDA_LAYER_RUNTIME],
