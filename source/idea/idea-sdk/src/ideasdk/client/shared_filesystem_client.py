...
class SharedFileSystemClient:
    ...
    def __init__(self, context: SocaContextProtocol,
        ...
        self.logger = context.logger('shared-filesystem-client')
    ...
    def list_onboarded_file_systems(self, request: ListOnboardedFileSystemsRequest) -> ListOnboardedFileSystemsResult:
        """
        List all onboarded file systems in the cluster
        """
        result = self.client.invoke_alt(
            namespace='FileSystem.ListOnboardedFileSystems',
            payload=request,
            result_as=ListOnboardedFileSystemsResult,
