...
def test_accounts_force_user_update_should_succeed(context, monkeypatch):
    """
    Update a valid group
    """
    monkeypatch.setattr(accounts, "get_user", lambda x: "xyz")
...
def test_accounts_get_group_with_invalid_type_group_name_should_fail():
    """
    get group with invalid type for group name
    """
    with pytest.raises(Exception) as exc_info:
        accounts.get_group(0)
    assert f"group name must be type string" in exc_info.value.args[0]
...
def test_accounts_get_group_with_invalid_type_group_name_should_fail():
    ...
    """
    with pytest.raises(Exception) as exc_info:
        accounts.get_group("group/name")
    assert (
        f"group name is invalid, must be 65 characters and cannot contain invalid characters"
...
def test_accounts_crud_create_group(context, monkeypatch):
    """
    external group
    """
...
def test_accounts_modify_group_with_empty_group_name_should_fail(context, monkeypatch):
    """
    modify group with empty name
    """
    with pytest.raises(Exception) as exc_info:
        group = accounts.update_group(
            {
                "title": "Dummy Group",
                "group_name": "",
                "ds_name": "",
    ...
    assert f"group_name is required" in exc_info.value.args[0]


def test_accounts_modify_group_with_db_group_none_should_fail(context, monkeypatch):
    """
    ...
    assert f"Group not found: xyz" in exc_info.value.args[0]


def test_accounts_modify_group_with_db_group_disabled_should_fail(context, monkeypatch):
    """
    ...
    assert f"Cannot modify a disabled group" in exc_info.value.args[0]


def test_accounts_modify_group_should_succeed(context, monkeypatch):
    """
    modify group
    """
    response = {"enabled": True, "group_name": "xyz", "title": "Dummy Group"}
    monkeypatch.setattr(accounts, "get_group", lambda x: response)
    monkeypatch.setattr(accounts, "update_group", lambda x: response)
    group = accounts.update_group(
        {
            "title": "Dummy Group",
            "group_name": "xyz",
...
def test_accounts_force_group_update_should_succeed(context, monkeypatch):
    """
    Update a valid group
    """
    monkeypatch.setattr(accounts, "get_group", lambda x: "xyz")
...
def test_accounts_crud_remove_user_from_groups_success(context, monkeypatch):
    """
...
mock_admin_group = {"name": "admin_group", "role": constants.ADMIN_ROLE}
...
def test_is_active_admin_admin_user(monkeypatch):
    ...
    monkeypatch.setattr(accounts, "get_user", lambda x: mock_inactive_admin)
    assert accounts.is_active_admin("admin_user") == False


def test_is_active_admin_regular_user(monkeypatch):
    monkeypatch.setattr(accounts, "get_user", lambda x: mock_regular_user)
    assert accounts.is_active_admin("regular_user") == False


def test_is_active_admin_user_in_admin_group(monkeypatch):
    monkeypatch.setattr(accounts, "get_user", lambda x: mock_user_in_admin_group)
    monkeypatch.setattr(accounts, "get_group", lambda x: mock_admin_group)
    assert accounts.is_active_admin("group_admin_user") == True


def test_is_active_admin_user_in_regular_group(monkeypatch):
    monkeypatch.setattr(accounts, "get_user", lambda x: mock_user_in_admin_group)
    monkeypatch.setattr(accounts, "get_group", lambda x: mock_regular_group)
    assert accounts.is_active_admin("group_admin_user") == False


def test_is_active_admin_user_in_multiple_groups(monkeypatch):
    user_with_multiple_groups = {
        "username": "multi_group_user",
        "role": "user",
        "additional_groups": ["regular_group", "admin_group"],
        "is_active": True,
        "enabled": True,
    }
    monkeypatch.setattr(accounts, "get_user", lambda x: user_with_multiple_groups)
    monkeypatch.setattr(
        accounts,
        "get_group",
        lambda x: mock_regular_group if x == "regular_group" else mock_admin_group,
    )
    assert accounts.is_active_admin("multi_group_user") == True
