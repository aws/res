...
class PipelineStack(Stack):
                    ...
                    iam.PolicyStatement.from_json(
                        {
                            ...
                            "Resource": [
                                f"arn:aws:dynamodb:{self.region}:{self.account}:table/{self.params.cluster_name}.cluster-settings",
                                f"arn:aws:dynamodb:{self.region}:{self.account}:table/{self.params.cluster_name}.modules",
                            ],
                        }
                    ),
                    iam.PolicyStatement.from_json(
    ...
    def get_destroy_step(self) -> pipelines.CodeBuildStep:
        codebuild_cloudformation_read_policy = iam.PolicyStatement(
            ...
            resources=[
                f"arn:aws:cloudformation:{self.region}:{self.account}:stack/Deploy-{INSTALL_STACK_NAME}/*",
                f"arn:aws:cloudformation:{self.region}:{self.account}:stack/{self.params.cluster_name}-bootstrap/*",
                f"arn:aws:cloudformation:{self.region}:{self.account}:stack/{self.params.cluster_name}-cluster/*",
                f"arn:aws:cloudformation:{self.region}:{self.account}:stack/{self.params.cluster_name}-metrics/*",
                f"arn:aws:cloudformation:{self.region}:{self.account}:stack/{self.params.cluster_name}-directoryservice/*",
                f"arn:aws:cloudformation:{self.region}:{self.account}:stack/{self.params.cluster_name}-identity-provider/*",
                f"arn:aws:cloudformation:{self.region}:{self.account}:stack/{self.params.cluster_name}-analytics/*",
                f"arn:aws:cloudformation:{self.region}:{self.account}:stack/{self.params.cluster_name}-shared-storage/*",
                f"arn:aws:cloudformation:{self.region}:{self.account}:stack/{self.params.cluster_name}-cluster-manager/*",
                f"arn:aws:cloudformation:{self.region}:{self.account}:stack/{self.params.cluster_name}-vdc/*",
                f"arn:aws:cloudformation:{self.region}:{self.account}:stack/{self.params.cluster_name}-bastion-host/*",
            ],
        )
    ...
    def get_publish_steps(
        ...
        codebuild_account_list_regions = iam.PolicyStatement(
            effect=iam.Effect.ALLOW,
            actions=["account:ListRegions"],
            resources=[f"arn:aws:account::{self.account}:account"],
        )
        return pipelines.CodeBuildStep(
            ...
            role_policy_statements=[
                ...
                codebuild_account_list_regions,
            ],
        )
