...
from ideadatamodel import exceptions, errorcodes, constants, ListRoleAssignmentsRequest, ListRoleAssignmentsResponse, RoleAssignment, SocaPaginator
...
class RoleAssignmentsDAO:
    ...
    def list_role_assignments(self, request: ListRoleAssignmentsRequest) -> ListRoleAssignmentsResponse:
        self.logger.info(f'list role assignments for {request.resource_key if Utils.is_empty(request.actor_key) else request.actor_key}')
        
        role_assignments = []

        if not Utils.is_empty(request.actor_key) and not Utils.is_empty(request.resource_key):
            role_assignment = self.get_role_assignment(request.actor_key, request.resource_key)
            ...
            while True:
                if not Utils.is_empty(request.actor_key):
                    query_params = { 'KeyConditionExpression': Key('actor_key').eq(request.actor_key) }
                    if pagination_key: query_params.ExclusiveStartKey = pagination_key
                    result = self.role_assignments_table.query(**query_params)
                elif not Utils.is_empty(request.resource_key):
                    query_params = { 'KeyConditionExpression': Key('resource_key').eq(request.resource_key), 'IndexName': GSI_RESOURCE_KEY }
                    if pagination_key: query_params.ExclusiveStartKey = pagination_key
                    result = self.role_assignments_table.query(**query_params)
    ...
    def get_projects_for_user(self, username: str, groups: List[str]) -> List[str]:
        ...
        role_assignments = self.list_role_assignments(ListRoleAssignmentsRequest(actor_key=f'{username}:user')).items
        ...
        for group_name in groups:
            role_assignments = self.list_role_assignments(ListRoleAssignmentsRequest(actor_key=f'{group_name}:group')).items
