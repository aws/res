      ...
      content: <Container
        header={
          <Header
            ...
            description="Permissions granted to this project role."
          >Permissions</Header>
        }
      >
        <SpaceBetween size="l">
    ...
    return (
      <IdeaAppLayout
        ...
        header={
            ...
            actions={
              <SpaceBetween size="m" direction="horizontal">
                <Button
                  variant="normal"
                  onClick={() => {
                    this.props.navigate("/cluster/permissions/project-roles/configure", { state: { isUpdate: true, profile: this.state.givenRole, fromPage: "view" } })
                  }}
                >
                  Edit
                </Button>
                <Button
                  ...
                  onClick={(e) => {
                    ...
                    if (numAffectedProjects === 0) {
                      this.setFlashbarMessage("in-progress", "Deleting 1 role");
                      this.authzClient().deleteRole({
                        role_id: this.state.givenRole?.role_id,
                      }).then((_) => {
                        this.props.navigate("/cluster/permissions");
                        setTimeout(() => {
                          this.setFlashbarMessage("success", "1 project role deleted successfully.")
                        }, 500);
                      }).catch((e) => {
                    ...
                    this.setFlashbarMessage(
                      ...
                      `Failed to delete ${this.state.givenRole?.name} role.`,
                      <Button
                        variant="normal"
                        iconName="external"
                        iconAlign="right"
                        onClick={() => {
                          this.props.navigate(`/cluster/permissions/project-roles/${this.state.givenRole?.role_id}`, { state: {
          ...
          >
            {this.state.givenRole?.name || "View project role"}
          </Header>
        }
        content={
          <SpaceBetween size="s">
