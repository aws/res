...
class ConfigurePermissionProfile extends Component<ConfigurePermissionProfileProps, ConfigurePermissionProfileState> {
      ...
      params={[
        {
          name: "name",
          title: "Role name",
          description: "Assign a name to the role",
        ...
        },
        {
          ...
          description: "Optionally add more details to describe the specific role.",
          data_type: "str",
          param_type: "text",
          multiline: true,
          validate: {
  ...
  submitForm() {
    ...
    if (!this.state.isUpdate) {
      ...
      .then((_) => {
        setTimeout(() => {
          this.setFlashbarMessage("success", "Successfully created project role.");
        }, 500);
        this.props.navigate("/cluster/permissions");
      })
      .catch((e) => {
        this.setFlashbarMessage("error", "Failed to create project role.");
      });
    } else {
      ...
      .then((_) => {
        if (this.state.fromPage === "view")
          this.props.navigate(`/cluster/permissions/project-roles/${this.state.profile?.role_id}`);
        else
          this.props.navigate("/cluster/permissions");
        setTimeout(() => {
          this.setFlashbarMessage(
            "success",
            `${this.state.profile?.name ?? "1"} role updated succesfully. This update impacted ${this.state.profile ? 
              ...
              <Button
                ...
                iconAlign="right"
                onClick={() => {
                  this.props.navigate(`/cluster/permissions/project-roles/${this.state.profile?.role_id}`, { state: {
      ...
      .catch((e) => {
        this.setFlashbarMessage("error", "Failed to update project role.");
      });
    }
  }
  ...
  render() {
    const breadCrumbItems = [
      ...
      {
        text: "Permission policy: Project roles",
      ...
      breadCrumbItems.push(...[
        {
          ...
          href: `#/cluster/permissions/project-roles/${this.state.profile!.role_id}`
        },
        {
          text: "Edit",
          href: `#/cluster/permissions/project-roles/${this.state.profile!.role_id}`,
    ...
    return (
      <IdeaAppLayout
        ...
        header={
          <Header variant="h1">
            {this.state.isUpdate ? `Edit ${this.state.profile!.name}` : "Create project role"}
          </Header>
        }
        content={
          <SpaceBetween size="l">
            ...
            <Container
              header={
                <Header variant="h2" description="Permissions granted to this project role.">Permissions</Header>
              }
            >
              <SpaceBetween size="l">
                <Box>
                  <h3 style={{ marginTop: 0, marginBottom: 0 }}>Project management permissions</h3>
                  <p style={{borderBottom: `2px solid ${Constants.DIVIDER_COLOR}` }}></p>
                </Box>
            ...
            <SpaceBetween size="m" direction="vertical" alignItems="end">
              <SpaceBetween size="m" direction="horizontal">
                  ...
                  }}>{this.state.isUpdate ? "Save changes" : "Create role"}</Button>
              </SpaceBetween>
            </SpaceBetween>
            <Modal
            ...
            >
              <p>Update <b>{this.state.profile?.name}</b> role?</p>
              <Alert
